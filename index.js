...
async function getSortedFilePaths(entryPoints, projectRoot) {
    ...
    const withEntries = topologicalSortedFiles.concat(entryPoints).map(f => fileNameToGlobalName(f, projectRoot))
...
async function getProjectRoot() {
    let configFilePath = await findUp(['truffle.js', 'truffle-config.js', 'buidler.config.js', 'buidler.config.ts'])
    if (!configFilePath) {
        throw new Error(`
